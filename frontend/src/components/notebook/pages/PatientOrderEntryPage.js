...
function PatientOrderEntryPage({
  ...
  // Load available tests from API, then filter by allowedTestIds from the notebook.
  // Empty allowedTestIds = not configured → no tests (do not fall back to full catalog).
  const loadAvailableTests = useCallback(() => {
    ...
    if (!allowedTestIds || allowedTestIds.length === 0) {
