import { isPermanentEquipment, isLotReceivable } from "./inventoryBehavior";
...
export const getItemTypeLabel = (type) => ITEM_TYPE_LABELS[type] || type || "";

export const formatItemTypesForDropdown = (types) =>
  (types || ["REAGENT", "EQUIPMENT"]).map((type) => ({
    id: type,
    text: getItemTypeLabel(type),
  }));

export const isExpiryTrackedType = (itemType) =>
  !isPermanentEquipment(itemType);
...
export const isLotReceivableType = (itemType) =>
  itemType && isLotReceivable(itemType);
